...
protected:
  ...
  //! always used for the Socket which is already exist
  Socket( FileDescriptor&& fd, int domain, int type, int protocol = 0 );

  //! Wrapper around [getsockopt(2)](\ref man2::getsockopt)(系统调用)
...
public:
  ...
  void connect( const Address& address ); // 由客户端发出，用于和服务器建立连接

  //! Shut down a socket via [shutdown(2)](\ref man2::shutdown)//通过系统调用关闭一个套接字
